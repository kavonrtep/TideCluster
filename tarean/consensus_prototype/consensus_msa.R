...
option_list <- list(
  ...
  make_option("--min-anchor-count", type = "integer", default = 0L,
              help = "Min anchor k-mer occurrences (<=0 = auto: max(4, floor(0.5 * L/m)))"),
...
diag_row <- function(res, extras) {
  ...
  row <- list(
    ...
    anchor_min_occ     = res$anchor_min_occ %||% 0L,
    expected_copies    = res$expected_copies %||% 0L,
